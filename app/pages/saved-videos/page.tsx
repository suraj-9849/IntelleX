...
import { Trash2, Search, Film, Clock, Plus, ChevronRight, ArrowLeft, Filter, PlayCircle, AlertTriangle } from 'lucide-react';
...
export default function SavedVideosPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col w-full overflow-x-hidden bg-gradient-to-br from-zinc-900 via-black to-zinc-900 text-white">
            ...
            <div className="flex gap-2">
              ...
              >
                <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <rect x="1" y="7" width="13" height="3" rx="1" fill="currentColor"/>
                  <rect x="1" y="12" width="13" height="1" rx="0.5" fill="currentColor"/>
                </svg>
              </Button>
            </div>
          ...
          <div className="rounded-xl border border-zinc-800 bg-black p-4 shadow-xl">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-white">{savedVideos.length}</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-white">
                  {savedVideos.reduce((acc, video) => acc + video.timestamps.length, 0)}
                </p>
              </div>
            </div>
          </div>
              ...
              <div>
                ...
                <p className="text-md font-medium text-white">
                  {savedVideos.length > 0 
                    ? new Date(parseInt(savedVideos.sort((a, b) => parseInt(b.id) - parseInt(a.id))[0].id)).toLocaleDateString() 
                    : "No videos"}
                </p>
              </div>
            </div>
                ...
                <div className="relative aspect-video cursor-pointer overflow-hidden bg-zinc-900">
                  ...
                  <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    <Link href={`/pages/video/${video.id}`}>
                      <Button size="icon" className="h-12 w-12 rounded-full bg-white/20 backdrop-blur-md hover:bg-white/30">
              ...
              >
                ...
                <div className="mt-4 flex-grow sm:mt-0">
                  <div className="flex items-center">
                    <h2 className="mr-2 text-lg font-semibold text-white">{video.name}</h2>
                    {video.timestamps.some(t => t.isDangerous) && (
                      <Badge className="bg-red-900/20 text-red-400">Sensitive</Badge>
                    )}
                  </div>
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-zinc-400">
                    <Badge className="bg-zinc-800">{video.timestamps.length} timestamps</Badge>
                    <span>•</span>
        ...
        {filteredVideos.length === 0 && (
          <div className="mt-8 flex flex-col items-center justify-center rounded-xl border border-zinc-800 bg-black p-16 text-center">
            ...
            <h3 className="mb-2 text-xl font-medium text-white">
              {searchTerm ? 'No videos match your search' : 'No videos in your library'}
            </h3>
