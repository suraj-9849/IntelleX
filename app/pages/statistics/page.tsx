...
export default function StatisticsPage() {
  const exportToCSV = () => {
    ...
    link.setAttribute(
      'download',
      `safety-statistics-${new Date().toISOString().split('T')[0]}.csv`
    );
  ...
  useEffect(() => {
    ...
    const dangerousByVideo = dangerousMoments.reduce(
      (acc: { [key: string]: number }, moment) => {
    ...
    const trendData = dangerousMoments.reduce(
      (acc: { [key: string]: number }, moment) => {
        const [hours, minutes] = moment.timestamp.split(':').map(Number);
        const interval =
          `${hours.toString().padStart(2, '0')}:${Math.floor(minutes / 15) * 15}`.padEnd(
            5,
            '0'
          );
    ...
    <div className='flex w-full flex-1 flex-col items-center gap-8 p-8'>
      <div className='flex w-full max-w-6xl flex-col gap-8'>
        ...
        <div className='grid grid-cols-1 gap-8 lg:grid-cols-3'>
          ...
          <div className='rounded-lg border bg-card p-6 shadow-sm'>
            <h2 className='mb-4 text-xl font-semibold'>
              Dangerous vs Non-Dangerous Moments
            </h2>
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id}>
                  {headerGroup.headers.map((header) => (
                    <th
                      key={header.id}
                      className='bg-muted/50 px-4 py-3 text-left'
                    >
        ...
        <div className='mt-8 rounded-lg border border-white/10 bg-white/5 p-6 backdrop-blur-sm'>
          <h2 className='mb-4 text-2xl font-semibold'>AI Analysis Summary</h2>
          {isLoadingSummary ? (
